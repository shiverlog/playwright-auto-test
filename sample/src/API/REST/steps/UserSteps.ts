...
export default class UserSteps {
  ...
  private get header() {
    return this.api.header
      .set(RESTConstants.CONTENT_TYPE, RESTConstants.CONTENT_JSON)
      .set(RESTConstants.ACCEPT, RESTConstants.CONTENT_JSON)
      .get();
  }
  ...
  public async post(
    ...
    await test.step(`Making POST call to ${operation}`, async () => {
      ...
      response = await this.api.rest.post(
  ...
  public async put(
    ...
    await test.step(`Making PUT call to ${operation}`, async () => {
      const requestJSON = await this.api.rest.createRequestBody(requestBodyFile, requestData);
      response = await this.api.rest.put(
        this.BASE_URL + endPoint,
        this.header,
        requestJSON,
        operation,
  ...
  public async verifyContentIsNotNull(
    response: RESTResponse,
    jsonPath: string,
    description: string,
  ) {
