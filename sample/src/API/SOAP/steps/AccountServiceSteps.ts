...
export default class AccountServiceSteps {
  ...
  public async request(
    ...
    await test.step(`SOAP request to ${operation}`, async () => {
      const requestHeaders = this.api.header
        .set(SOAPConstants.CONTENT_TYPE, SOAPConstants.CONTENT_TEXT)
        .get();
      response = await this.api.soap.post(
        endPoint,
        requestHeaders,
        requestBody,
        requestData,
  ...
  public async verifyResponseContains(
    response: SOAPResponse,
    xpath: string,
    result: string,
    operation: string,
