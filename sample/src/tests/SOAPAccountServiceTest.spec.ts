...
test(`${loginData.TestID} - ${loginData.Description}`, async ({ gData }) => {
  ...
  gData.set(
    'token',
    await account.getResponseContent(response, "//*[local-name()='token']", loginData.Operation),
  );
  gData.set(
    'userId',
    await account.getResponseContent(response, "//*[local-name()='userId']", loginData.Operation),
  );
});
...
test(`${logoutData.TestID} - ${logoutData.Description}`, async ({ gData }) => {
  ...
  const response = await account.request(
    logoutData.EndPoint,
    logoutData.RequestBody,
    requestData,
    logoutData.Operation,
  );
...
test(`${searchData.TestID} - ${searchData.Description}`, async ({}) => {
  ...
  const response = await account.request(
    ...
    { countryStartingWith: searchData.Result },
    searchData.Operation,
  );
  await account.verifyResponseContains(
    response,
    searchData.Xpath,
    searchData.Result,
    searchData.Operation,
