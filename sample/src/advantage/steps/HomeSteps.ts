...
export default class HomeSteps {
  ...
  public async enterLoginDetails(userName: string, password: string) {
    await test.step(`Enter login credentials as ${userName} & ${password}`, async () => {
      ...
      await this.ui
        .checkbox(HomePage.REMEMBER_ME_CHECKBOX, HomePageConstants.REMEMBER_ME_CHECKBOX)
        .check();
  ...
  public async validateLogin(userName: string) {
    await test.step(`Verify that user is successfully logged in as ${userName}`, async () => {
      const user = await this.ui
        .element(HomePage.LOGGED_IN_USER, HomePageConstants.USER_NAME)
        .getTextContent();
  ...
  public async validateInvalidLogin(errorMessage: string) {
    await test.step(`Verify that error message ${errorMessage}`, async () => {
      const user = await this.ui
        .element(HomePage.SIGN_IN_ERROR_MESSAGE, HomePageConstants.SIGN_IN_ERROR_MESSAGE)
        .getTextContent();
  ...
  public async navigateToCreateAccount() {
    await test.step(`Navigate to Create Account page`, async () => {
      await this.ui.element(HomePage.USER_ICON, HomePageConstants.USER_ICON).click();
      await this.ui
        .element(HomePage.CREATE_NEW_ACCOUNT_LINK, HomePageConstants.CREATE_NEW_ACCOUNT_LINK)
        .click();
    });
  }
  /**
   ...
   * @param subject
   */
  public async enterContactUsDetails(
    category: string,
    product: string,
    email: string,
    subject: string,
    ...
    await test.step(`Entering Contact Us details`, async () => {
      await this.ui
        .dropdown(HomePage.CATEGORY_DROPDOWN, HomePageConstants.CATEGORY_DROPDOWN)
        .selectByVisibleText(category);
      await this.ui
        .dropdown(HomePage.PRODUCT_DROPDOWN, HomePageConstants.PRODUCT_DROPDOWN)
        .selectByVisibleText(product);
      await this.ui.editBox(HomePage.EMAIL_TEXTBOX, HomePageConstants.EMAIL_TEXTBOX).fill(email);
      await this.ui
        .editBox(HomePage.SUBJECT_TEXTAREA, HomePageConstants.SUBJECT_TEXTAREA)
        .fill(subject);
    });
  }
  /**
  ...
  public async verifySuccessMessage(message: string) {
    await test.step(`Verifying Success Message of Contact Us`, async () => {
      const actualMessage = await this.ui
        .element(HomePage.CONTACT_US_MESSAGE, HomePageConstants.CONTACT_US_MESSAGE)
        .getTextContent();
  ...
  public async searchProduct(product: string) {
    await test.step(`Searching for product '${product}'`, async () => {
      ...
      await (
        await this.ui
          .editBox(HomePage.SEARCH_TEXTBOX, HomePageConstants.SEARCH_TEXTBOX)
          .type(product)
      ).keyPress(HomePageConstants.ENTER_KEY);
      await this.ui
        .element(HomePage.SEARCH_CLOSE_IMAGE, HomePageConstants.SEARCH_CLOSE_IMAGE)
        .click();
    });
  }
  /**
  ...
  public async navigateToManagementConsole() {
    ...
    await test.step(`Navigate to Management Console screen`, async () => {
      ...
      newPage = await this.ui.switchToNewWindow(
        HomePage.MANAGEMENT_CONSOLE_LINK,
