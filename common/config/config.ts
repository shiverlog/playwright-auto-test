...
// 개별 결과 파일 (날짜별 저장)
const getLogFile = (poc: POCType): string =>
  `${LOG_PATH(poc)}/${poc}_${getCurrentTimestamp()}.json`;
const getTestResultFile = (poc: POCType): string =>
  `${TEST_RESULT_PATH(poc)}/${poc}_test-result_${getCurrentTimestamp()}.json`;
const getScreenshotFile = (poc: POCType): string =>
  `${SCREENSHOT_PATH(poc)}/${poc}_screenshot_${getCurrentTimestamp()}.png`;
const getVideoFile = (poc: POCType): string =>
  `${VIDEO_PATH(poc)}/${poc}_video_${getCurrentTimestamp()}.mp4`;
const getTraceFile = (poc: POCType): string =>
  `${TRACE_PATH(poc)}/${poc}_trace_${getCurrentTimestamp()}.zip`;
