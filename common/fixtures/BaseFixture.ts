...
export abstract class BaseFixture {
   ...
   * 공통 전처리 작업
   */
  public async beforeAll(poc: POCKey): Promise<void> {
    const logger = Logger.getLogger(poc) as winston.Logger;
   ...
   * 공통 후처리 작업
   */
  public async afterAll(poc: POCKey): Promise<void> {
    const logger = Logger.getLogger(poc) as winston.Logger;
   ...
   * 공통 로그 출력
   */
  protected log(message: string, poc: POCKey = 'PC'): void {
    const logger = Logger.getLogger(poc) as winston.Logger;
  ...
  public async runTests(poc: POCType = 'ALL'): Promise<void> {
    const isAll = poc === 'ALL';
    const targetPOCs: POCKey[] = isAll ? ALL_POCS : [poc as POCKey];
   ...
   * POC별 테스트 준비 작업
   */
  protected abstract prepare(poc: POCKey, ...args: any[]): Promise<void>;
}
