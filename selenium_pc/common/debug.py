...
class Debug():
    ...
    def get_log2(self):
        ...
        # file_handler_pass = logging.FileHandler(filename=self.Driver.path+'/result/debug/' + datetime.now().strftime('%Y%m%d') + '_info.log',encoding='utf-8', mode='w')
        # file_handler_fail = logging.FileHandler(filename=self.Driver.path+'/result/debug/' + datetime.now().strftime('%Y%m%d') + '_info.log',encoding='utf-8', mode='w')
    ...
    def screenshot(self):
        print(f"result/screenshot/img{self.Driver.getCount()}.png")
        ...
        self.driver.save_screenshot(f"result/screenshot/img{self.Driver.getCount()}.png")            # 스크린 샷 저장
        Slack.setFilePath(f"result/screenshot/img{self.Driver.getCount()}.png")
        # self.count = self.count + 1
        self.Driver.Counting()
        return self.Driver.getCount()


    # def screenshot_log(self,path:str='C:/dev/remotePC_batchfiles/pubsub/selenium_pc/result/debug',func_name:str=None):

    #     # self.driver.save_screenshot("result/screenshot/" + datetime.now().strftime('%Y%m%d') +"_" + func_name + str(var) +"_fail.png")            # 스크린 샷 저장
    #     print(f"{path}/{func_name}.png")
    #     print(f"func_name =>>> {func_name}")
    #     self.driver.save_screenshot(f"{path}/{func_name}_{datetime.now().strftime('%Y%m%d')}.png")            # 스크린 샷 저장
    #     return
