class InternetIptvPage():
    ...
    def iptv(self):
        ...
        try :
            ...
            #     check_iptv_item[iptv_item_list[num+num].get_property('innerText')]=iptv_item_list[num+(1*(num+1))].get_property('innerText')
            ...
            # btn = self.FC.loading_find_css_pre(self.FC.var['iptv_el']['가입상담_신청_팝업_닫기'])
            # self.FC.move_to_click(btn)

            # 온라인 가입 데이터 정상 출력 확인
            # product_value_list=self.FC.loading_find_csss(self.FC.var['iptv_el']['온라인가입_상품명'])
