...
# class NewSessionStart(InvalidSessionIdException):
#     def __init__(
...
#     def create_new_session(self,befoDriver:AppDriver):
#         driver = AppDriver(port=befoDriver.PORT,device_name='Z_FLIP4_삼성인터넷')
#         test=Tests(driver)
...
class Debug():
    ...
    def get_log(self):
        ...
        formats = {
                ...
                logger.FAIL: '%(asctime)s - [FAIL] - [%(real_funcName)s] - %(message)s',
    ...
    # 에러 발생 시 스크린 샷 
    # 크롬 드라이버 다운그레이드 시, 스크린샷 이슈 있음 -> 컨텍스트 전환 필요(참고 https://discuss.appium.io/t/capture-screenshot-in-webview-context/877, https://bugs.chromium.org/p/chromedriver/issues/detail?id=792)
    ...
    def screenshot(self):
        try:
            ...
            print("="*50)
            self.driver.save_screenshot(f"{self.Driver.path}/result/screenshot/img_{self.Driver.udid}_{self.Driver.getCount()}.png")            # 스크린 샷 저장
    ...
    def screenshot_del(self):
        ...
        # 스크린샷 경로    
        path_png = f'{self.Driver.path}/result/screenshot/*.png'
