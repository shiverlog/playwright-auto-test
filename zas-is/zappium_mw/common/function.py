...
class Function():
    ...
    def chrome_clear(self,version:str=''):
        ...
        try:
            ...
            print(e)
    def samsung_clear(self,version:str=''):
        '''
        삼성 인터넷 cache clear
        '''
        if version == '':
            cmd=f"adb -s {self.Driver.udid} shell pm clear com.sec.android.app.sbrowser.beta"
        elif version == 'beta':
            cmd=f"adb -s {self.Driver.udid} shell pm clear com.sec.android.app.sbrowser"
    ...
    def loading(self,css):
        ...
        while True:
            try:
                ...
                # print(loading_elem.get_property('outerHTML'))
                count+=1
                if count>max_count:
    ...
    def move_to_click(self,el,JS_switch=False):
        ...
        if JS_switch is False:
            # self.action.w3c_actions.pointer_action.move_to(el).click()
            # self.action.perform()
            self.action.move_to_element(el).click().pause(1).perform()
            self.action.reset_actions()
            self.wait_loading()
