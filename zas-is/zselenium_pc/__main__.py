...
class Tests():
    def __init__(self,WebDriver:WebDriver):
        ...
        # self.ChatbotPage=ChatbotPage(WebDriver,self.FC)

    # 테스트 케이스 작성
    def TestCase_1(self):
        test_result = [] # 기능 수행 결과
        Slack.send_slack_title("PC 실행 시작")
        self.FC.modal_ck()
        self.DBG.screenshot_del()
        
        test_result.append(self.LoginPage.u_plus_login_retry(2, "uplus"))
        test_result.append(self.MainPage.mainpage())  
        test_result.append(self.MainPage.mainpage())  
        test_result.append(self.MainPage.mainpage_myinfo())
        ...
        Slack.send_slack_title("PC 실행 완료")
...
if __name__ == '__main__' :
    driver=WebDriver()
    TESTS=Tests(driver)
    res = TESTS.TestCase_1()
    driver.driver.close()
    driver.driver.quit()
    driver.quit_debugger_chrome(res)
