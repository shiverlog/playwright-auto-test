class MainPage():
    ...
    def do_login_v2(self, login_type:str):
        '''
        로그인 2.0
        U+ID, 카카오, 네이버, 구글 로그인
        login_type : [v2_uplus, v2_kakao, v2_naver, v2_google]
        '''
        self.FC.gotoHome()

        try:
            self.FC.movepage(self.FC.var['login_el']['login_btn'],address=self.FC.var['login_el']['login_v2_url'])
            self.FC.wait_loading()

            dict = {
                "v2_uplus" : [os.environ['UPLUS_ID'], os.environ['UPLUS_PW']]
            }

            if self.FC.loading_find_css(self.FC.var['login_el']['v2_login_list']):
                self.FC.loading_find_css(self.FC.var['login_el'][f'{login_type}_select_login_img']).get_property('parentElement').click()
                self.FC.wait_loading()
