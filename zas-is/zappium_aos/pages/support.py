class SupportPage():
    ...
    def support(self):
        ...
        try:
            ...
            result.clear()
            self.FC.move_to_element(self.FC.loading_find_css_pre(self.FC.var['support_el']['스스로_해결_가이드']))
            ...
            #     print(info_list_el[num].get_property('innerText'))
            #     container = self.FC.loading_find_css_pre('div.submain-section > div.c-section-md')
            #     self.FC.scroll_center(container)
            #     # self.FC.action.move_to_element(info_list_el[num]).click().perform()
            #     # self.FC.action.reset_actions()
            #     info_list_el[num].click()
            ...
            if self.FC.driver.current_context != "WEBVIEW_com.lguplus.mobile.cs":
