...
class DirectPage():
    ...
    def direct(self):
        ...
        try:
            ...
            # USIM 신규가입
            self.FC.loading_find_css_pre(self.FC.var['direct_el']['USIM_신규가입']).click()
            self.FC.wait_loading()
            assert self.FC.var['direct_el']['전용요금제_가입하기_url'] in self.FC.driver.current_url, self.DBG.logger.debug("다이렉트 > USIM 신규가입 버튼 페이지 이동 실패")
            self.FC.goto_url(self.FC.var['direct_el']['url'])
    ...
    def direct2(self):
        ...
        try:
            ...
            # 모달 제거
            time.sleep(3)
            if self.FC.loading_find_css('div#directDialogPopup') != None:
            ...
            ## sec2(cont-02) 콘텐츠 확인
            self.FC.action.move_to_element(self.FC.loading_find_css_pre('div.cont-02')).perform()
            self.FC.action.reset_actions()
            ...
            # self.FC.action.move_to_element(plan_list[random_num]).click().perform()
            # self.FC.action.reset_actions()
            ...
            if self.FC.var['direct_el']['url'] != self.FC.driver.current_url:
                self.FC.goto_url(self.FC.var['direct_el']['url'])
                if self.FC.loading_find_css('div#directDialogPopup'):
                    self.FC.loading_find_css_pre('div#directDialogPopup button.ui_modal_close').click()
            ...
            self.FC.swipe('div.cont-03')
            time.sleep(2)
            self.FC.swipe(self.FC.loading_find_css_pre('div.cont-03 dl'))
            btns=self.FC.loading_find_csss('div.cont-03 a')
            btns[1].click()
            assert self.FC.var['uth_el']['url'] in self.FC.loading_find_css_pre('div.uth_allwrap').get_property('baseURI'),self.DBG.logger.debug("다이렉트 > sec3 콘텐츠 및 액션 정상 동작 실패")
            # while True:
            #     if self.FC.var['direct_el']['url'] != self.FC.driver.current_url:
            #         self.FC.driver.back()
            #     if 'https://m.lguplus.com/' == self.FC.driver.current_url:
            #         self.FC.movepage(self.FC.var['direct_el']['direct'],address=self.FC.var['direct_el']['url'])   # 상단 햄버거 버튼 클릭 동작
            #     else:
            #         break
            self.FC.driver.get(self.FC.var['direct_el']['url'])
            if self.FC.loading_find_css('div#directDialogPopup'):
                        self.FC.loading_find_css_pre('div#directDialogPopup button.ui_modal_close').click()
            ...
            while True:
                if self.FC.var['direct_el']['url'] != self.FC.driver.current_url:
                    self.FC.goto_url(self.FC.var['direct_el']['url'])
                if self.FC.var['common_el']['url'] == self.FC.driver.current_url:
                    self.FC.movepage(self.FC.var['direct_el']['direct'],address=self.FC.var['direct_el']['url'])   # 상단 햄버거 버튼 클릭 동작
                if self.FC.var['direct_el']['url'] == self.FC.driver.current_url:
                    break
