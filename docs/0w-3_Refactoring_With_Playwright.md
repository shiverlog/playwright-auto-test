...
기존의 Selenium, Appium 으로 구축된 AS-IS 프로젝트에서 Playwright를 기반한 자동화 프로젝트로 TO-BE 하여야 이유는 다음과 같다.
...
1. 다만, 자동화 담당 인원의 Playwright 및 POM 구조, 그리고 TypeScript(TS) 언어에 대한 이해도가 부족한 상태이므로, POC(Proof of Concept) 진행을 위해 일정 기간의 학습 및 실습이 필요하다. 이에 따라, 5주 동안 POC를 진행하며, Playwright을 활용한 PC 자동화 환경 구축 및 모듈화 적용을 목표로 한다.
