...
# 파일/디렉토리 상관없이 selenium_mw이 존재하지 않으면 git sclone, 존재하면 git pull 
...
then
	...
	#pip install -r C:/dev/remotePC_batchfiles/pubsub/selenium_mw/requirements.txt;
	pip list;
fi > 'C:/dev/remotePC_batchfiles/selenium_mw_batchfiles/batchfiles_result/step2_git_clone_result.log' 2>&1
