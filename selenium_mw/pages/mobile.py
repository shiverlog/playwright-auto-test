...
class MobilePage():
    ...
    def mobile(self):
        ...
        try:
            ...
            check_link = (self.FC.var['common_el']['home_url'] + path_link if path_link.startswith('/') else path_link).replace('/main', '')
            ...
            self.FC.driver.get(self.FC.var['mobile_el']['url'])
            ...
            self.FC.is_exists_element_click(self.FC.loading_find_css_pre(self.FC.var['mobile_el']['휴대폰_title']))
            self.FC.wait_loading()
            ...
            self.FC.driver.get(self.FC.var['mobile_el']['url'])
            self.FC.move_to_element(self.FC.loading_find_css(self.FC.var['mobile_el']['휴대폰_title']))
            # 휴대폰 탭 메뉴 정상 노출
            ...
            self.FC.scroll_center(list[random_num])
            list[random_num].click()
            ...
            # self.FC.scroll_el(self.FC.var['mobile_el']['휴대폰결합상품'],'true')
            # self.FC.wait_loading()
            # button=self.FC.loading_find_css_pre(self.FC.var['mobile_el']['가입상담 신청 버튼'])
            # self.FC.move_to_element(button)

            # # 휴대폰/인터넷/IPTV 드롭다운 선택 후 가입 상담 신청 시, 임의 선택한 데이터 정상 노출 확인(선택한 데이터:check_list)
            # select_list=self.FC.loading_find_csss(self.FC.var['mobile_el']['select'])
            # for select in select_list:
            #     current_select=Select(select)
            #     random_num=random.randrange(0,len(current_select.options))
            #     if random_num !=0:
            #         self.FC.action.move_to_element(select).click().perform()
            #         self.FC.action.reset_actions()
            ...
            # price_value=self.FC.loading_find_csss(self.FC.var['mobile_el']['휴대폰 결합 계산 결과값'])
            # check_value=[]
            ...
            # title= self.FC.loading_find_css(self.FC.var['common_el']['popup_title']).get_property('innerText')  #상담 신청
            # assert "상담 신청" in title, self.DBG.logger.debug("모바일 > 서브메인 > 휴대폰 결합 할인 계산 영역의 가입상담 신청 팝업 > 콘텐츠 정상 노출 실패 ")
            # self.FC.wait_loading()
            # price_list=self.FC.loading_find_csss(self.FC.var['mobile_el']['할인_정보'])
            ...
            # self.FC.loading_find_css(self.FC.var['mobile_el']['가입상담_창닫기']).click()     # 창닫기
            # self.FC.driver.switch_to.window(self.FC.driver.window_handles[0])   
            # self.FC.wait_loading()

            # 태블릿/스마트워치/노트북 영역 기능 및 콘텐츠 정상 노출 확인
            # 태블릿/스마트워치/노트북  타이틀 링크 이동 확인
            ...
            for n in range(0,random_num):                                           
                self.FC.action.click_and_hold(carousel).move_by_offset(-100,0).release().perform()                                 
                # self.FC.action.move_to_element_with_offset(carousel,160,50).click_and_hold(None).move_by_offset(-200,0).release().perform()                                                      
                self.FC.action.reset_actions()
            ...
            self.FC.driver.execute_script('return arguments[0].scrollIntoViewIfNeeded();',list[random_num])
            ...
            self.FC.driver.execute_script('window.scrollBy(0, 500);')
            ...
            self.FC.driver.execute_script('window.scrollBy(0, 200);')
            check_text=['일반 구매 후기']
            ...
            self.DBG.print_dbg("모바일 > 페이지 정상 노출 및 기능 확인",False)
            self.FC.close_popup(self.FC.driver.window_handles)
            return False

        else :
            self.DBG.print_dbg("모바일 > 페이지 정상 노출 및 기능 확인")
            return True
    ...
    def mobile_plan(self):
        ...
        try:
            ...
            for key in datas.keys():
                ...
                if self.FC.loading_find_css_pre(self.FC.var['mobile_el']['요금제비교함_신청하기_버튼']).get_attribute('disabled'):
            ...
            self.DBG.print_dbg("모바일 > 모바일 > 모바일 요금제 > 5G/LTE > 페이지 정상 노출 및 기능 확인",False)
            self.FC.close_popup(self.FC.driver.window_handles)
            return False

        else :
            self.DBG.print_dbg("모바일 > 모바일 > 모바일 요금제 > 5G/LTE > 페이지 정상 노출 및 기능 확인")
            return True
